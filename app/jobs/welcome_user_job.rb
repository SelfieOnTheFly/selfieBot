class WelcomeUserJob < ActiveJob::Base
  ...
  def perform(user)
    ...
    client.direct_message(user.tw_handle, 'Nous allons configurer ton compte ensemble')
    client.direct_message(user.tw_handle, 'A tout moment tu peux repondre "skip" pour passer à la question suivante')
